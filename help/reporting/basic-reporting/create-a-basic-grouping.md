...
* **分组的用途：**&#x200B;分组是Workfront中的一个关键报表元素，用于根据共享信息（如完成日期、项目组合或项目群）组织项目列表。  
* **创建分组：**&#x200B;您最多可以使用三个级别的条件创建自定义分组。 例&#x200B;如，项目可以首先按项目组合分组，然后按项目群分组，以改善组织效果。  
* **无法覆盖编辑和保存分组：**&#x200B;内置分组，但您可以将编辑内容另存为新分组。 自&#x200B;定义分组应该具有清晰的描述性名称，以便轻松识别。  
* **共享分组：**&#x200B;分组可以与其他用户共享，具有默认的“查看”权限，允许他们使用和共享分组，但不能编辑它。 “&#x200B;管理”权限允许编辑和删除。  
* **删除分组：**&#x200B;删除您创建的分组也会从与其共享该分组的用户列表中将其删除。 共享&#x200B;的分组显示在其他用户的“与我共享”部分下。  
