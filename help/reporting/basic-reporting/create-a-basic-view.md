...
此视频介绍如何在Workfront中创建和自定义视图，以显示有关列表中项的特定信息，如项目、任务、问题和文档。 视图&#x200B;允许用户查看详细信息，如名称、描述、状态以及与项目相关的其他字段。  
...
* **内联编辑**：列表视图中的某些字段可以直接更新，而无需打开单个项目，从而加快更改速度。  
* **从头开始创建视图**：用户可以通过添加预算、实际成本和进度状态等相关列来设计视图，以满足特定需求，例如跟踪项目运行状况。  
...
将鼠标悬停在 [!UICONTROL Status Icons] 列中的每个图标上，查看它们所代表的内容。 如果它们呈灰显状态，则意味着任务没有注释、文档、批准流程等。如果图标以彩色显示，则至少有一个项目与任务相关联。 您可以单击注释或文档图标以转到该项目。
