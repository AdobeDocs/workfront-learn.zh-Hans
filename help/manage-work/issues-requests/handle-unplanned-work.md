---
title: 有效处理计划外工作
description: 通过有效跟踪和解决问题、利用日志记录工具、实施最佳实践、促进沟通并将问题无缝转化为任务以简化执行，改进Workfront中的项目管理。
activity: use
team: Technical Marketing
feature: Work Management
thumbnail: handle-unplanned-work.jpeg
type: Tutorial
role: User
level: Beginner
last-substantial-update: '2024-06-26T00:00:00.000Z'
recommendations: noDisplay,catalog
jira: KT-10070
exl-id: 43caddaf-5fd8-4580-9a37-59b5ecc57c91
product_v2:
  - id: c4a86a5d-6562-4fc6-aa00-bfa25833aed9
feature_v2:
  - id: a0dacc9f-0e23-495b-8e9f-a77c2e60b40c
subfeature_v2:
  - id: c10f2e93-7a58-4212-aa24-684c265ebe76
role_v2:
  - id: b69b2659-1057-424e-8fc5-ed9e016dc554
level_v2:
  - id: e8ccd51f-da0d-4e3b-939b-e30d5ebb1ea5
autotag-review: '2026-05-06T14:57:12.678Z'
source-git-commit: 9f00285646af281d6c4d93eb792f4c38eedefb40
workflow-type: tm+mt
source-wordcount: 260
ht-degree: 11%

---

# 有效处理计划外工作

该视频强调了沟通在项目管理中的重要性，并强调了问题作为促进沟通的关键工具的作用。 它&#x200B;解释了更新与问题之间的区别，指出问题更适合在项目完成之前需要分配、跟踪和解决的工作。 此视频&#x200B;提供了有关如何记录问题的分步指南。  


>[!VIDEO](https://video.tv.adobe.com/v/3419488/?quality=12&learn=on&enablevpops=1)

## 关键要点

* **问题与更新：**&#x200B;对于项目完成之前需要分配、跟踪和解决的任务，使用问题，因为与更新相比，它们更容易监控和组织。  
* **日志记录问题：**&#x200B;可以通过“问题”选项卡在任务或项目级别记录问题。 可以设置&#x200B;优先级，也可以为分配和更新配置电子邮件通知。
* **问题管理的最佳实践：**&#x200B;经常监视问题，创建自定义报告以提高效率，并为未解决的问题提供明确的解释，以避免重复的问题并生成历史记录。  
* **鼓励沟通：**&#x200B;在回应中表达感激之情，以鼓励团队成员记录问题并有效沟通，确保更顺畅的项目管理。  
* **将问题转化为任务：**&#x200B;可以将问题转化为任务并集成到项目时间线中，以便更好地规划和执行。  


## 有关此主题的推荐教程

* [开始管理项目](/help/manage-work/projects/getting-started-manage-a-project.md)
* [了解项目沟通](/help/manage-work/projects/understand-project-communication.md)
* [将问题/请求转化为项目](/help/manage-work/issues-requests/create-a-project-from-a-request.md)
* [将问题/请求转化为任务](/help/manage-work/issues-requests/convert-issues-to-other-work-items.md)
