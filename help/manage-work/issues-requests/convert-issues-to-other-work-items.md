---
title: 将问题或请求转换为任务
description: 使用可编辑的名称、无缝的自定义表单附件、灵活的项目选择、转换选项和同步的状态更新，轻松地将问题转换为Workfront中的任务，以简化工作流。
activity: use
team: Technical Marketing
feature: Work Management
type: Tutorial
role: User
level: Intermediate
last-substantial-update: '2024-02-28T00:00:00.000Z'
recommendations: noDisplay,catalog
jira: KT-10069
exl-id: 1fd4d862-e44b-4c50-9663-70e727f6e9b7
product_v2: id: c4a86a5d-6562-4fc6-aa00-bfa25833aed9
feature_v2: id: a0dacc9f-0e23-495b-8e9f-a77c2e60b40c
subfeature_v2: id: c10f2e93-7a58-4212-aa24-684c265ebe76
role_v2: id: b69b2659-1057-424e-8fc5-ed9e016dc554
level_v2: id: b5a62a22-46f7-4f0d-b151-3fc640bef588
autotag-review: '2026-05-06T14:55:57.327Z'
source-git-commit: 9f00285646af281d6c4d93eb792f4c38eedefb40
workflow-type: tm+mt
source-wordcount: 240
ht-degree: 17%

---

# 将问题或请求转换为任务

该视频演示了如何将问题或请求转化为任务。

>[!VIDEO](https://video.tv.adobe.com/v/3427605/?quality=12&learn=on&enablevpops=1)

## 关键要点

* **简单转换流程：**&#x200B;要将问题/请求转换为任务，请打开问题，单击三个圆点菜单，然后选择“转换为任务”。 默认情况下&#x200B;，会复制任务名称和描述，但可以对其进行编辑。  
* **目标项目选择：**&#x200B;您可以选择任务的目标项目，可以是问题所在的默认项目，也可以是您有权访问的其他项目。  
* **自动表单附件：**&#x200B;与问题关联的自定义表单会自动添加到任务中，确保所有字段值无缝转移。  
* **转换选项：**&#x200B;键选项包括：
   * 将原始问题与任务一起保持活动状态。  
   * 允许请求者访问任务。  
   * 保留问题的任务计划完成日期。  
* **链接状态更新：**&#x200B;如果保留原始问题，则其状态将根据任务的进度自动更新，从而确保问题和任务之间的同步。  


>[!NOTE]
>
>您不能将问题添加到项目时间表中，因为它们代表“规划外的工作”。 项目时间线用于“规划的工作”，即任务。

## 有关此主题的推荐教程

* [查找和管理请求](/help/manage-work/issues-requests/find-requests.md)
* [将问题/请求转化为项目](/help/manage-work/issues-requests/create-a-project-from-a-request.md)
* [处理规划外的工作](/help/manage-work/issues-requests/handle-unplanned-work.md)

