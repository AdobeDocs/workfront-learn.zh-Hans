---
title: 创建工作角色和费率卡
description: 了解如何创建工作角色和费率卡，以及如何从项目附加和删除费率卡。
activity: use
team: Technical Marketing
feature: Work Management
type: Tutorial
role: User
level: Intermediate
jira: KT-20259
last-substantial-update: '2026-05-08T00:00:00.000Z'
recommendations: noDisplay,catalog
doc-type: video
product_v2:
  - id: c4a86a5d-6562-4fc6-aa00-bfa25833aed9
feature_v2:
  - id: d968a1bc-9a90-4926-a531-bcf272c32aad
  - id: a0dacc9f-0e23-495b-8e9f-a77c2e60b40c
subfeature_v2:
  - id: f0dd7b45-76b5-49d4-afe3-39f436b6fbd3
role_v2:
  - id: b69b2659-1057-424e-8fc5-ed9e016dc554
level_v2:
  - id: b5a62a22-46f7-4f0d-b151-3fc640bef588
autotag-review: '2026-05-06T14:51:17.520Z'
source-git-commit: ee03983e89e8c99e2fbdc12b108f3fb33d6d8e72
workflow-type: tm+mt
source-wordcount: 205
ht-degree: 13%

---

# 创建工作角色和费率卡

此视频介绍如何创建工作角色和费率卡，以及如何从项目附加和删除费率卡。


>[!VIDEO](https://video.tv.adobe.com/v/3486517/?quality=12&learn=on&enablevpops=1)

## 关键要点

* **工作角色结构项目任务：**&#x200B;工作角色表示任务所需的技能，可以根据用户的能力和可用性分配给用户。
* **费率卡定义记帐费率：**&#x200B;费率卡列出工作角色的记帐费率，并充当项目记帐的与客户合同。
* **灵活的费率管理：**&#x200B;工作角色和费率卡都允许使用有效日期设置成本和记帐费率，支持基于时间的定价更改。
* **费率卡覆盖：**&#x200B;将费率卡附加到项目时，其费率会覆盖工作角色费率，但项目特定的覆盖不受影响。
* **轻松的附件和更新：**&#x200B;可以从项目中附加或删除费率卡，并且可以将更新推送到现有项目以确保帐单准确无误。

## 有关此主题的推荐教程

* [了解绩效指标](/help/manage-work/project-finances/understand-performance-metrics.md)
* [设置任务收入和成本默认值](/help/manage-work/project-finances/set-up-task-revenue-and-cost-defaults.md)
* [设置汇率](/help/manage-work/project-finances/set-up-exchange-rates.md)
* [设置费用类型](/help/manage-work/project-finances/set-up-expense-types.md)
* [了解财务访问权限](/help/manage-work/project-finances/understand-financial-access.md)
