---
title: 使用基线、快照和自动化报表进行主项目跟踪
description: 在Workfront中释放历史可见性的强大功能。 虽然Workfront在显示实时项目更新方面表现出色，但了解您的任务在过去各个阶段的表现则需要更深入的了解。 在本视频中，您将了解三种跟踪历史任务数据的方法：使用基线、捕获快照和设置自动任务报告电子邮件。
feature: Work Management
type: Tutorial
role: User
level: Intermediate
activity: deploy
team: Technical Marketing
last-substantial-update: 2026-04-20T00:00:00Z
jira: KT-20256
doc-type: video
source-git-commit: e90a7ab4dcb8341ab1f942c50a018ad429550570
workflow-type: tm+mt
source-wordcount: '237'
ht-degree: 0%

---

# 使用基线、快照和自动化报表进行主项目跟踪

此视频重点介绍使用基线、快照和已发送报告进行有效项目跟踪、历史分析和利益相关者沟通的重要性。


>[!VIDEO](https://video.tv.adobe.com/v/3483617/?quality=12&learn=on&enablevpops)

## 要点

* **基线提供历史参考点：**&#x200B;基线捕获项目在关键里程碑（例如，启动、主要重新计划）时的状态，并用于衡量原始计划的进度。
* **自动和手动基线创建：**&#x200B;当项目状态更改为“当前”时，将自动创建基线，也可以手动创建基线。 并非所有项目字段都在基线中捕获。
* **快照捕获所有项目数据：**&#x200B;与基线不同，快照是手动创建的，用于捕获所有项目、任务、问题和自定义字段，可在任何时间点提供全面的记录。
* **灵活的比较和报告：**&#x200B;用户可以使用自定义报告和视图将当前项目数据与基线或快照进行比较，从而详细分析更改和进度。
* **自动定期报告发送的报告：**&#x200B;通过“发送报告”功能，用户可以计划和接收定期电子邮件报告，从而随着时间的推移自动跟踪和存档项目数据。
