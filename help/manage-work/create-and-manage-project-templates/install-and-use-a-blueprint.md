---
title: 安装和使用Blueprint
description: 了解管理员如何安装Blueprint以及用户如何将其投入使用。
activity: use
feature: Work Management
type: Tutorial
role: User, Admin
level: Beginner
team: Technical Marketing
last-substantial-update: 2026-2-12
recommendations: noDisplay,catalog
jira: KT-20314
doc-type: video
source-git-commit: bb1f0cb236ff68aaa89a6d6f11b5e3f4bba06dcc
workflow-type: tm+mt
source-wordcount: '172'
ht-degree: 24%

---

# 安装和使用Blueprint

了解管理员如何安装Blueprint以及用户如何将其投入使用。 在此示例中，我们将安装“面向新用户的Workfront培训”Blueprint，并使用其创建的模板和视图作为培训核对清单来帮助启用Workfront中的用户。


>[!VIDEO](https://video.tv.adobe.com/v/3479833/?quality=12&learn=on&enablevpops=0)

## 要点

* 如何选择Blueprint并确定其是否已安装。 大多数用户都可使用Standard许可证完成此操作。
* 如何安装Blueprint并共享其创建的对象。 这需要系统管理员的服务。
* 如何查找和使用通过安装Blueprint创建的新项目模板。


## 有关此主题的推荐教程

* [创建项目模板并了解 Blueprint](/help/manage-work/create-and-manage-project-templates/create-a-project-template.md)
* [直接从模板创建项目](/help/manage-work/create-and-manage-project-templates/create-a-project-directly-from-a-template.md)
* [共享项目模板](/help/manage-work/create-and-manage-project-templates/share-a-project-template.md)
* [复制现有项目](/help/manage-work/manage-projects/copy-an-existing-project.md)
* [停用项目模板](/help/manage-work/create-and-manage-project-templates/deactivate-a-project-template.md)
* [在项目模板中编辑项目团队](/help/manage-work/create-and-manage-project-templates/edit-the-project-team-in-a-project-template.md)
