...
围绕完成类似工作、需要访问Workfront相同部分等的一组人员构建布局。这不仅能够确保用户有权访问其工作所需的Workfront部分，而且意味着用于系统和组管理员创建和维护的布局模板更少。
...
主菜单中的选项，最常用的菜单项位于显著位置（或按字母顺序排列主菜单）
左侧面板菜单部分，顺序便于执行工作流
标准和自定义字段在主页中可见，允许用户完成分配，无需深入研究任务/问题即可获得所需信息
