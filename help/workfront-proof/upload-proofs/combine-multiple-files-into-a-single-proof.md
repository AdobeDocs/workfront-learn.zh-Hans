---
title: 将多个文件合并为一份校样
description: 了解如何在  [!DNL  Workfront] 中通过将多个文件合并为一个校对来简化校对过程。
activity: use
...
将多个文件组合成一个校对通常可以使校对过程更加简化。

当各个文件相关联或属于整体可交付结果的一部分，并且所有文件都需要由同一个人在同一截止日期前审阅时，“合并”选项非常有用。

例如，创意团队设计了一本小册子。 四位设计师制作了这些页面，并将其保存为单独的 PDF。 如果每个设计师将这些作为单独的校样上传，则审核人将会有四个单独的校样需要审阅。 此外，确保小册子的各个部分能够拼凑在一起会更加困难。

解决方案——让一个人上传所有 PDF，并在上传时将它们合并为一份校样。 这使得审阅者可以将小册子看作一个整体，而不是分开的部分。

若要合并校样：

1. 打开附加该校样的项目、任务或问题的 [!UICONTROL Documents] 部分。
1. 单击&#x200B;**新增>验证**。
1. 将文件拖放到上传区域，或浏览到这些文件。 [!DNL Workfront] 最多支持合并 50 个文件。
1. 在&#x200B;**单个校对**&#x200B;下，切换将所有兼容的文件合并到一个校对的选项。
1. 为合并的校样输入一个名称。 这是必填项。
...
上传校样后，您将在 [!UICONTROL Documents] 选项卡中看到它的 ZIP 文件。

查看合并的文件不需要任何其他操作。 只需像往常一样单击 [!UICONTROL Open Proof]，校样便会在校样查看器中打开。

![校样查看器的图像，其中可以看到多页校样。](assets/combine-proofs-2.png)
