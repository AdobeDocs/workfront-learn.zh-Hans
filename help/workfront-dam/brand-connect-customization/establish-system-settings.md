...
* **在搜索引擎中显示此门户？** — 是否希望在Internet上运行搜索时显示[!UICONTROL Brand Connect] URL？
* **允许可嵌入的链接吗？** — 可嵌入的链接是否来自[!UICONTROL Brand Connect]？ 这将在 [!UICONTROL Share] 菜单中添加一个 [!UICONTROL Get Links] 面板，图为资源提供可嵌入的链接。
* **是否启用/禁用Social？** — 如果启用Social（选中框），则用户可以在资产上查看和评论。
* **启用/禁用下载和查看计数？** — 启用后，用户可以查看某个资源已下载的次数以及它包含多少注释。
